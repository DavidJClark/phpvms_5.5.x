...
class googleChart {
    ...
    function draw($noReturn = true, $append = '&chxr.png') {
        ...
        if ($this->showGrid) {
            if (($gridx = $this->showGrid) == 1) {
                ...
                $items += $sets;
                if ($sets >= 1) $gridx = round($items / $sets); //get count of items per set
                else  $gridx = 0;
                if ($gridx == 0) $gridx = $this->showGrid;
                if ($this->type == 'lc') $gridx--;
            }
            if ($this->minMaxLabel) $gridy = round(100 / ($this->minMaxLabel));
            else  $gridy = 25;
    ...
    function setLabels($labels, $side = 'left') {
        ...
        if ($this->type[0] == 'p') { //for pie charts
            if ($labels == 'minmax') { //for auto settings min/max range
                ...
                if ($this->labels) $this->labels .= '|'; //do pie chart label
            $this->labels .= $labels;
            ...
            if ($labels == 'minmax') { //auto set min/max range
                ...
                if ($this->minMaxLabel == 0) $this->minMaxLabel = 1;
    ...
    function loadData($data) {
        ...
        if ($this->negativeMode)
            foreach ($data as & $datum) $datum = abs($datum);
        $this->max = max($data);
        $this->min = min($data);
